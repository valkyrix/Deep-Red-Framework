...
class singleNMAPXMLParser():
    ...
    def parse_input(self, input_string, ips):
        ...
        #logging.debug("Parsing NMAP for single ips info")
        for e1 in root:
            if e1.tag == "host":
                host = e1
                ...
                for os in os:
                    ...
                    name = os.attrib["name"]
                    acc = os.attrib["accuracy"]
                    s = dumps(name + ". Prediction accuracy: {0}".format(acc))
                    info[ip_address].append(s)
