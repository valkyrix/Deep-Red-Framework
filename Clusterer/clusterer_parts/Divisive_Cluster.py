...
from clusterer_parts.display import display_vector_index_details, display_shared_vector_indeces_details


def query_for_split_decision(inds1, inds2, vectorizer, vectors, vector_names):
...
class Cluster:
    ...
    def divide_decision(self, vectorizer, vectors, vector_names, min_pts=2):
        ...
        if query_for_split_decision(self.index[np.argmax(self.distance_matrix[indx])], self.index[indx], vectorizer, vectors, vector_names):
